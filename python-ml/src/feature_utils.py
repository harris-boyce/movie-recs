...
class FeatureNameMapper:
    ...
    def create_feature_documentation(
        ...
        output_path: str = "data/processed/feature_documentation.md",
    ) -> None:
...
class ScalingUtilities:
    ...
    @staticmethod
    def scale_feature_groups(
        features: np.ndarray,
        feature_names: List[str],
        group_scalers: Dict[str, str] = None,
...
class CategoryEncoder:
    ...
    def encode_multi_label(
        multi_values: List[List[str]], category_map: Optional[Dict[str, int]] = None
    ) -> Tuple[np.ndarray, Dict[str, int]]:
        """
...
class FeatureSelector:
    ...
    def reduce_dimensionality(
        features: np.ndarray, n_components: int = 100, method: str = "pca"
    ) -> Tuple[np.ndarray, Any]:
        """
...
class FeatureAnalyzer:
    ...
    def analyze_feature_correlations(
        features: np.ndarray, feature_names: List[str], threshold: float = 0.8
    ) -> Dict[str, Any]:
        """
    ...
    def feature_importance_summary(
        feature_names: List[str], importances: np.ndarray, top_k: int = 20
    ) -> Dict[str, Any]:
        """
...
def save_feature_metadata(
    ...
    output_path: str = "data/processed/feature_metadata.json",
) -> None:
    """
        ...
        "features": [
            {
                "name": name,
                "description": (
                    feature_descriptions.get(name, name_mapper.get_description(name))
                    if feature_descriptions
                    else name_mapper.get_description(name)
                ),
