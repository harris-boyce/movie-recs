...
class PersonInfo(BaseModel):
    ...
    role: str = Field(..., regex=r"^(actor|director|writer|producer|composer|cinematographer|editor)$")
    birth_year: Optional[int] = Field(None, ge=1800, le=2025)
    gender: Optional[str] = Field(None, regex=r"^(male|female|non-binary|other|unknown)$")
...
class Ratings(BaseModel):
    ...
    def validate_distribution(cls, v, values):
        if v and 'count' in values:
            ...
            if total_distributed > values['count']:
...
class Metadata(BaseModel):
    ...
    country: str = Field("US", max_length=2, regex=r"^[A-Z]{2}$")
    ...
    production_companies: Optional[List[str]] = Field(None, max_items=20)
    keywords: Optional[List[str]] = Field(None, max_items=50)
    imdb_id: Optional[str] = Field(None, regex=r"^tt\d{7,8}$")
    tmdb_id: Optional[int] = Field(None, ge=1)
    
    @validator('production_companies', 'keywords')
...
class Movie(BaseModel):
    ...
    genres: List[Genre] = Field(..., min_items=1, max_items=10, description="Movie genres")
    cast: Optional[List[PersonInfo]] = Field(None, max_items=50, description="Cast information")
    crew: Optional[List[PersonInfo]] = Field(None, max_items=30, description="Crew information") 
    ...
    @root_validator
    def validate_cast_crew(cls, values):
        """Validate cast and crew information consistency."""
        cast = values.get('cast', []) or []
        crew = values.get('crew', []) or []
        ...
        if release_year:
            for person in cast + crew:
                if person.birth_year and person.birth_year > release_year - 5:
                    # Allow 5-year buffer for child actors
                    if release_year - person.birth_year < 5:
                        continue
                    raise ValueError(f'Person birth year {person.birth_year} inconsistent with release year {release_year}')
    ...
    def validate_metadata_consistency(cls, values):
        ...
        if metadata:
            ...
            if metadata.budget and metadata.revenue:
                # Revenue should typically be >= budget (allowing for losses)
                if metadata.revenue > 0 and metadata.budget > metadata.revenue * 10:
...
class MovieCollection(BaseModel):
    ...
    movies: List[Movie] = Field(..., min_items=1)
    ...
    def validate_total_count(cls, v, values):
        if 'movies' in values and len(values['movies']) != v:
            raise ValueError('Total count must match actual number of movies')
        return v
    
    @root_validator
    def validate_collection(cls, values):
        """Validate collection-level constraints."""
        movies = values.get('movies', [])
        ...
        if movies:
            ...
            values['data_sources'] = list(sources)
...
def get_schema_info() -> Dict[str, Any]:
    ...
    return {
        ...
        'person_roles': ['actor', 'director', 'writer', 'producer', 'composer', 'cinematographer', 'editor'],
        'validation_rules': {
