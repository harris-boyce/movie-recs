...
from .schema import (
    Movie, MovieCollection, ValidationResult, QualityThresholds, 
    create_movie_from_dict, Genre, Language
)
...
class DataValidator:
    ...
    def validate_dataset(self, movies: List[Dict[str, Any]]) -> Tuple[ValidationResult, List[Movie]]:
        ...
        completeness_rate = result.summary['valid_movies'] / result.summary['total_movies']
        ...
        logger.info(f"Validation complete: {result.summary['valid_movies']}/{result.summary['total_movies']} movies valid")
    ...
    def _analyze_genre_diversity(self, df: pd.DataFrame, movies: List[Movie]) -> Dict[str, float]:
        ...
        if genre_counts:
            ...
            shannon_diversity = -sum(
                ...
                for count in genre_counts.values() if count > 0
            )
        else:
        ...
        underrepresented = [
            genre for genre, pct in genre_percentages.items() 
    ...
    def _generate_bias_recommendations(self, metrics: BiasMetrics) -> List[str]:
        ...
        if underrepresented:
            recommendations.append(f"Increase representation of underrepresented genres: {', '.join(underrepresented[:3])}")
    ...
    def generate_html_report(self, 
                           ...
                           movies: List[Movie],
                           output_path: str = "data/reports/data_quality_report.html") -> None:
